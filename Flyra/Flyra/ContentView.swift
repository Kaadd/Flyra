...
struct ContentView: View {
    ...
    private func fetchFlight() async {
        ...
        do {
            let flight = try await FlightService.shared.fetchFlight(flightID: flightID.trimmingCharacters(in: .whitespaces))
            await MainActor.run {
            ...
            await MainActor.run {
                ...
                case .cannotConnectToHost:
                    errorMessage = "Cannot connect to server. Make sure the server is running on localhost:8000"
                case .badServerResponse:
                    errorMessage = "Server error. Flight not found or server returned an error."
                case .badURL:
            ...
            await MainActor.run {
                errorMessage = "Failed to parse flight data: \(decodingError.localizedDescription)"
                isLoading = false
            }
        } catch {
            await MainActor.run {
                errorMessage = "Failed to fetch flight: \(error.localizedDescription)"
                isLoading = false
            }
        }
    }
